...
const Expenses: React.FC = () => {
    ...
    const handleDuplicate = (t: any) => {
        ...
        setExpenseToEdit({ ...duplicateData, description: `${t.description} (Cópia)` });
        setIsModalOpen(true);
    };
    ...
    const { transactions: visibleExpenses, hiddenCount, hiddenValue } = useMemo(() => {
    ...
    const sortedExpenses = useMemo(() => {
        let sortableItems = [...visibleExpenses];
        if (sortConfig !== null) {
    ...
    }, [visibleExpenses, sortConfig, data]);

    const stats = useMemo(() => {
        const activeExpenses = visibleExpenses.filter((t: Transaction) => !t.isIgnored);
        const pending = activeExpenses.filter((t: Transaction) => FinancialEngine.getEffectiveTransactionStatus(t, referenceMonth) !== 'confirmed').reduce((acc: number, t: Transaction) => acc + t.value, 0);
        const paid = activeExpenses.filter((t: Transaction) => FinancialEngine.getEffectiveTransactionStatus(t, referenceMonth) === 'confirmed').reduce((acc: number, t: Transaction) => acc + t.value, 0);
        return { pending, paid, total: pending + paid };
    }, [visibleExpenses, referenceMonth]);
    ...
    const summaryPanel = (
        <div className="sys-summary-widget">
            <div className="sys-summary-widget-header">Resumo de Saídas</div>
            <div className="sys-summary-block">
                <span className="sys-summary-block-title">Contas a Pagar</span>
                <span className="sys-summary-block-value color-yellow">{formatCurrency(stats.pending)}</span>
            </div>
            <div className="sys-summary-block">
                <span className="sys-summary-block-title">Contas Pagas</span>
            ...
            <div className="sys-summary-block" style={{ borderBottom: 'none', paddingTop: '8px' }}>
                <span className="sys-summary-block-title" style={{ fontSize: '13px' }}>Total de Despesas</span>
                <span className="sys-summary-block-value color-red" style={{ fontSize: '24px' }}>{formatCurrency(stats.total)}</span>
            </div>
        </div>
    );
    ...
    return (
        <PageLayout title="Despesas" actions={headerActions} summaryPanel={summaryPanel}>
            <div className="sys-card" style={{ padding: '20px' }}>
                <div className="sys-filters-row">
                    <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                        <div style={{ padding: '6px 14px', backgroundColor: 'rgba(239, 68, 68, 0.1)', color: 'var(--sys-red)', borderRadius: '8px', fontSize: '12px', fontWeight: 700, display: 'flex', alignItems: 'center', gap: '6px' }}>
                            <TrendingDown size={14} /> FILTRADO POR SAÍDAS
                        </div>
                    </div>
                                    ...
                                    <tr key={t.id} className="sys-table-row-compact" style={{ opacity: t.isIgnored ? 0.4 : 1 }}>
                                                ...
                                                <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                                                    ...
                                                    {t.isRecurring && t.recurrence?.installmentsCount && (
                                                        <span style={{ fontSize: '10px', color: isConfirmed ? 'var(--sys-green)' : 'var(--sys-blue)', backgroundColor: isConfirmed ? 'var(--sys-bg-green)' : 'var(--sys-bg-blue)', padding: '2px 8px', borderRadius: '4px', fontWeight: 800 }}>
                                                            {FinancialEngine.getInstallmentText(t, referenceMonth)} {isConfirmed ? 'PAGA' : 'PENDENTE'}
                                                        </span>
                                                    )}
                                                </div>
                                        ...
                                        <td>
                                            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                                <div className="sys-status-dot" style={{ backgroundColor: 'var(--sys-red)' }} />
                                        ...
                                        <td className="col-value">
                                            <span className="sys-financial-val-sm color-red">
            ...
            {showDeletePrompt && (
                <div style={{ position: 'fixed', inset: 0, background: 'rgba(15, 23, 42, 0.4)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
                    <div className="sys-card" style={{ width: 380, textAlign: 'center', padding: '32px' }}>
                        ...
                        <h3 style={{ fontSize: '18px', fontWeight: 800, marginBottom: '8px' }}>Excluir despesa repetida?</h3>
                        <p style={{ fontSize: '14px', color: '#64748b', marginBottom: '28px' }}>Esta despesa faz parte de uma série recorrente. Como deseja excluí-la?</p>
